...
import { whoAmI, logOut, saveScore, type UserSession } from './lib/api';
...
export default function App() {
  ...
  const resultStyles = {
    ...
    Tie: 'bg-amber-100 text-amber-700 border-amber-400',
    Loss: 'bg-red-100 text-red-700 border-red-400',
  }[result];
      ...
      <div className="absolute top-4 right-4 flex items-center gap-3 text-sm text-amber-800">
        {session ? (
          ...
          <>
            ...
            <Button size="sm" variant="outline" onClick={() => setIsGuest(false)}>
              Exit Guest Mode
            </Button>
          </>
        )}
      ...
      <div className="text-center mb-8 relative">
        ...
        {gameEnded && (
          <motion.div
            ...
            className="absolute -top-4 right-0 md:-right-16"
          >
            <span className={`inline-block text-xl px-4 py-2 rounded-full border-2 shadow-lg ${resultStyles}`}>
              {result}
            </span>
          </motion.div>
        )}
      ...
      <div className="mb-8">
        ...
        {session && (
          <p className="text-center text-sm text-amber-700 mt-2">
            Last saved score: ${session.score}
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            {boxes.map((box) => (
              <motion.div
                ...
                style={{ cursor: box.isOpen ? 'default' : `url(${keyCursor}) 16 16, pointer` }}
